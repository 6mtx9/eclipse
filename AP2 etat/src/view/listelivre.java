...
public class listelivre {
	...
	private void initialize() {
		...
		for (int i=0;i!=mainMVC.getM().getListLivre().size();i++) {
			...
			tableauliste.add("ISBN : "+mainMVC.getM().getListLivre().get(i).getISBN()+" Titre : "+mainMVC.getM().getListLivre().get(i).getTitre()+" Prix : "+mainMVC.getM().getListLivre().get(i).getPrix()+" Disponibilité : "+emprunt+ " Etat : "+mainMVC.getM().getListLivre().get(i).getEtat().getEtat());
		}
